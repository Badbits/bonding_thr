...
/* for param_lock/unlock facilities */
#include "bond_thr.h"
...
static ssize_t bonding_show_speed(struct device *d,
                                   struct device_attribute *attr,
                                   char *buf)
{
	struct slave *slave;
	...
	bond_for_each_slave(bond, slave, i) {
		...
		res += sprintf(buf + res, "%s:%d;", slave->dev->name, slave->link_speed);
	}
	bond_thr_param_unlock(bond);
	read_unlock(&bond->lock);
	if (res)
		buf[res-1] = '\n'; /* eat the leftover delimiter */
	return res;
}

static void store_link_speed(struct bonding *bond, const char *ifname, u32 new_value) 
{
	struct slave *slave;
	int i;

	bond_for_each_slave(bond, slave, i) {
		if (!strcmp(slave->dev->name, ifname)) {
			slave->link_speed = new_value;
			break;
		}
	}
}

typedef void (*store_link_speed_func)(struct bonding *bond, const char *ifname, u32 new_value);

static ssize_t bonding_parse_speed(struct bonding *bond,
                                    const char *buf, size_t count, store_link_speed_func store_func)
{
	int  rc, val, ret = count;
	char token[IFNAMSIZ+16];
	char * beg, * end, * delim;
	...
	do {
		...
		strncpy(token, beg, sizeof(token)/sizeof(token[0]));
		if(!(delim = strchr(token, ':'))) {
			printk(KERN_INFO DRV_NAME ": %s: Unable to parse link speed, delimiter \':\'not found: \'%s\'\n", 
				bond->dev->name, token);
		...
		rc = sscanf(delim + 1, "%d", &val);
		if (rc != 1) {
			printk(KERN_INFO DRV_NAME ": %s: Unable to parse link speed, token: \'%s\'\n", 
				bond->dev->name, token);
		...
		*delim = '\0';
		if (val <= 0) {
			printk(KERN_INFO DRV_NAME ": %s: Unable to set link speed for %s to %d, it must be greater then 0\n", 
				bond->dev->name, token, val);

	                ret = -EINVAL;
	                goto out;
		}

		/* update corresponding slave */
		if (store_func)
			(*store_func)(bond, token, val);
...
static ssize_t bonding_store_speed(struct device *d,
                                    struct device_attribute *attr,
                                    const char *buf, size_t count)
{
	int ret = count;
	...
	ret = bonding_parse_speed(bond, buf, count, NULL);
	if (ret < 0)
		goto out; 
	...
	ret = bonding_parse_speed(bond, buf, count, &store_link_speed);
...
static DEVICE_ATTR(link_speed, S_IRUGO | S_IWUSR, bonding_show_speed, bonding_store_speed);
