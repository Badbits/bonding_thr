...
#define DRV_DESCRIPTION	"Ethernet Channel Bonding Driver (modified for [GAF799182])"
...
struct slave {
	...
	u32    link_speed;    /* used in BOND_MODE_THROUGHPUT mode */
	u32    tx_hist;       /* used in BOND_MODE_THROUGHPUT mode */
	u16    speed;
	u8     duplex;
...
struct bonding {
	...
	spinlock_t thr_param_lock; /* used in BOND_MODE_THROUGHPUT mode */
	s8       kill_timers;
	s8	 send_grat_arp;
	s8	 send_unsol_na;
	s8	 setup_by_slave;
