...
{	"balance-thr",		BOND_MODE_THROUGHPUT},
{	NULL,			-1},
};
...
static const char *bond_mode_name(int mode)
{
	static const char *names[] = {
		...
		[BOND_MODE_THROUGHPUT] = "load balancing (throughput)",
	};

	if (mode < 0 || mode > BOND_MODE_THROUGHPUT)
		return "unknown";
...
int bond_enslave(struct net_device *bond_dev, struct net_device *slave_dev)
{
	...
	else if (bond->params.mode == BOND_MODE_THROUGHPUT) {

		res = bond_thr_init_slave(bond, new_slave);
		if (res) {
			goto err_close;
		}
	}
...
static void bond_info_show_master(struct seq_file *seq)
{
	...
	if (bond->params.mode == BOND_MODE_THROUGHPUT) {
		bond_thr_param_lock(bond);
		bond_for_each_slave(bond, slave, i) {	
			seq_printf(seq, "slave %s, hist: %d, link_speed: %d, link ratio: %d\n",
				slave->dev->name, slave->tx_hist, slave->link_speed, slave->tx_hist/slave->link_speed);
		}
...
static int bond_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	case BOND_MODE_THROUGHPUT:
		return bond_xmit_throughput(skb, dev);
	case BOND_MODE_ACTIVEBACKUP:
...
void bond_set_mode_ops(struct bonding *bond, int mode)
{
	...
	case BOND_MODE_ROUNDROBIN:
		break;
	case BOND_MODE_THROUGHPUT:
		printk(KERN_INFO DRV_NAME  ": set BOND_MODE_THROUGHPUT\n");
		break;
	case BOND_MODE_ACTIVEBACKUP:
		break;
	case BOND_MODE_XOR:
